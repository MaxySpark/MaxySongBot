...
function getMusic (songUrl,songName,sender,callback) {
    request({
            url : songUrl,
            gzip : true
        }, (err, res, body)=> {
...
function getDownloadUrl(searchUrl,sender,callback){
    request({
        url : searchUrl,
        gzip : true
    }, (err, res, body) => {
        if(err) throw err;
            ...
            getMusic(mainUrl,songTitle,sender,callback);
        }
    });
}

module.exports = function(s_name,sender,callback){
    var q_url = "https://www.youtube.com/results?search_query="+encodeURIComponent(s_name);
    getDownloadUrl(q_url,sender,callback);
}
