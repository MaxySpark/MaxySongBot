...
function sendMsg(data) {
    ...
    var msgLongLink = data.url;
    var params = {
        longUrl : msgLongLink
    }
    // urlShort(sendId,sendName,msgLongLink,send);
    
     urlshortener.url.insert({ resource: params, key: config.API_KEY }, function(err, result) {
        send(sendId,sendName,result.id);
        });
...
function sendMsgBlank(sendId) {
    request({
            ...
            json : {
                ...
                "message":{
                    "text" : "Invalid Song Name (Do not send emoji or stickers)"
                }
            }
        }, (err,res,body) => {
            if(err) throw err;
